...
print(
    f"{'QUERY TYPE':<16} | {'QUERY TEXT':<45} | {'TOP MATCH FACT (TRUNCATED)':<30} | {'COSINE':<7}"
)
print("-" * 105)
...
for q_type, q_text in queries:
    ...
    scores = [
        (cosine_similarity(q_vec, f_vec), facts[i])
        for i, f_vec in enumerate(fact_vectors)
    ]
    scores.sort(key=lambda x: x[0], reverse=True)
    best_score, best_fact = scores[0]
    print(
        f"{q_type:<16} | {q_text[:45]:<45} | {best_fact[:30]:<30}... | {best_score:.4f}"
    )
...
v_pop = embed_one(
    "git stash pop applies stashed changes and deletes them from stash list"
)
v_apply = embed_one(
    "git stash apply applies stashed changes and keeps them in stash list"
)
sim_opposing = cosine_similarity(v_pop, v_apply)
print(
    f"Cosine similarity between 'git stash pop' vs 'git stash apply' facts: {sim_opposing:.4f}"
)

v_dup1 = embed_one("To create a release with GitHub CLI: gh release create <tag>")
v_dup2 = embed_one(
    "Create a release in GitHub CLI with command: gh release create <tag>"
)
sim_dup = cosine_similarity(v_dup1, v_dup2)
print(
    f"Cosine similarity between minor paraphrases of same fact (dedup test): {sim_dup:.4f}"
)
...
def hybrid_search(query: str, dense_w: float = 0.6, sparse_w: float = 0.4):
    ...
    for i, fact in enumerate(facts):
        ...
        if "stash" in fact or ("release" in query and "release" in fact):
            ...
            print(
                f"    Dense: {d_score:.4f} | Sparse (norm BM25): {s_score:.4f} | Hybrid: {final_score:.4f}"
            )
            ...
            for name in names:
                ...
                if name in TIER_2:
                    if node in tree.body:
                        return (
                            False,
                            f"Tier 2 module '{name}' imported at global/module scope (must be function-scoped)",
                        )
