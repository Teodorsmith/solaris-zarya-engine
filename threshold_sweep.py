...
def query_corpus(query: str, dense_w: float = 0.6, sparse_w: float = 0.4) -> Tuple[float, float, float, str]:
...
print("Facts present: Release create, Release view, PR create, Stash pop/apply, Rebase, Merge")
print("=" * 110)

related_unknown_queries = [
    ("What is the maximum file upload size for GitHub releases?", "GitHub Release Limits"),
    ("How do I delete an asset from a GitHub release using CLI?", "GitHub Release Asset Delete"),
...
print(f"{'QUERY TEXT':<60} | {'DENSE':<7} | {'SPARSE':<7} | {'HYBRID':<7} | {'LANDS IN ZONE (Hybrid)'}")
print("-" * 110)
for q, desc in related_unknown_queries:
    d, s, h, f = query_corpus(q)
    zone = "PASS (>=0.80) [DANGER]" if h >= 0.80 else ("GREY [0.65, 0.80)" if h >= 0.65 else "REJECT (<0.65)")
    print(f"{q[:60]:<60} | {d:.4f} | {s:.4f} | {h:.4f} | {zone}")

print("\n" * 1)
print("=" * 110)
print("EXPERIMENT 2: POP VS APPLY PHRASING SPREAD (DEDUPLICATION THRESHOLD 0.95 STRESS TEST)")
print("=" * 110)
...
for p in pop_phrasings:
    ...
    for a in apply_phrasings:
        ...
        if sim > max_cross_sim:
            max_cross_sim = sim
        if sim >= 0.90:
            print(f"{p[:45]:<45} | {a[:45]:<45} | {sim:.4f} {'[HIGH!]' if sim>=0.93 else ''}")
...
print(f"{'CATEGORY':<12} | {'QUERY TEXT':<45} | {'DENSE':<7} | {'SPARSE':<7} | {'HYBRID':<7} | {'CLASSIFICATION'}")
print("-" * 110)
...
print(f"{'PASS THRESHOLD':<15} | {'TRUE HITS PASS RATE':<22} | {'UNKNOWN QUERIES LEAKING AS PASS (FALSE POSITIVES)':<45}")
print("-" * 110)

for t in [0.70, 0.73, 0.75, 0.78, 0.80, 0.82, 0.85]:
    true_hits = [h for cat, is_ans, h in hybrid_scores_all if cat in ("TRUE HIT", "OPPOSING")]
    unknowns = [h for cat, is_ans, h in hybrid_scores_all if cat in ("UNKNOWN", "UNRELATED")]
    ...
    print(f"{t:<15.2f} | {hit_pass_rate:<6.1f}% ({sum(1 for h in true_hits if h >= t)}/{len(true_hits)})           | {unknown_leak_rate:<6.1f}% ({leaked_count}/{len(unknowns)} leaked into pass zone)")
