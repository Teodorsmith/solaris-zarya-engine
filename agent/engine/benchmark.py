...
def run_reasoning_benchmark(rest: str, brain_manager, self_model) -> None:
    console = Console()
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Do not write ceilings to self-model"
    )
    ...
    console.print(
        f"[bold cyan]ZPD Reasoning Calibration[/bold cyan] (Brain: {brain_manager.brain.__class__.__name__})"
    )

    runner = ZPDRunner(brain_manager)
    ...
    if args.dry_run:
    ...
    elif isinstance(brain_manager.brain, MockBrain):
        console.print(
            "[yellow]Notice: MockBrain detected. Not persisting ZPD ceilings.[/yellow]"
        )
    elif self_model:
        ...
        console.print(
            "[yellow]Warning: no self-model loaded, cannot persist results.[/yellow]"
        )
