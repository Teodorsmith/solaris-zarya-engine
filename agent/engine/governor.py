...
class PermissionGovernor:
    ...
    def request_permission(
        ...
        if is_autonomous and depth > 2:
            self._log_decision(
                "DENIED",
                f"Autonomous mode depth cap exceeded (depth {depth} > 2) for goal: {goal.description}",
            )
            return False
        ...
        if not is_autonomous and depth > 4:
            self._log_decision(
                "DENIED",
                f"Supervised mode depth cap exceeded (depth {depth} > 4) for goal: {goal.description}",
            )
            return False
        ...
        if response in ("y", "yes"):
            self._log_decision(
                "USER_APPROVED", f"User approved action: {action_description}"
            )
            return True
        else:
            self._log_decision(
                "USER_DENIED",
                f"User denied action (response='{response}'): {action_description}",
            )
            return False
    ...
    def request_skill_write_permission(
        ...
        if is_autonomous:
            self._log_decision(
                "DENIED",
                f"Autonomous skill write disabled without supervisor approval for: {skill_name} ({file_path})",
            )
            return False
        ...
        if response in ("y", "yes"):
            self._log_decision(
                "USER_APPROVED",
                f"User approved skill file write: {skill_name} ({file_path})",
            )
            return True
        else:
            self._log_decision(
                "USER_DENIED",
                f"User denied skill file write (response='{response}'): {skill_name} ({file_path})",
            )
            return False
    ...
    def request_file_write_permission(
        ...
        if is_autonomous:
            self._log_decision(
                "DENIED", f"Autonomous file write denied for: {file_path}"
            )
            return False
        ...
        if response in ("y", "yes"):
            ...
            self._log_decision(
                "USER_DENIED",
                f"User denied file write (response='{response}'): {file_path}",
            )
            return False
