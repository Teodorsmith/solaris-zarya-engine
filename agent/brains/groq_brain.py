...
class GroqBrain(BaseBrain):
    ...
    def _get_fallback_brain(self) -> BaseBrain | None:
        import os
        ...
        if openai_key:
            ...
            return OpenAIBrain(
                api_key=openai_key, model=model, rpm_limit=rpm, base_url=base_url
            )
    ...
    def generate(self, prompt: str, **kwargs) -> str:
            ...
            try:
                with httpx.Client(timeout=self.timeout) as client:
                    ...
                    if response.status_code in self.retryable_statuses:
                        ...
                        logger.warning(
                            f"Groq API returned {response.status_code}. Retrying in {sleep_duration:.2f}s..."
                        )
        ...
        if fallback:
            logger.warning(
                f"Groq API retries exhausted. Failing over to {fallback.__class__.__name__}..."
            )
            return fallback.generate(prompt)
    ...
    def embed(self, text: str) -> list[float]:
        raise NotImplementedError(
            "Embeddings should be handled by FastEmbed via EmbeddingEngine, not Groq."
        )
