...
class OpenAIBrain(BaseBrain):
    ...
    def generate(self, prompt: str) -> str:
            ...
            try:
                with httpx.Client(timeout=self.timeout) as client:
                    ...
                    if response.status_code in self.retryable_statuses:
                        ...
                        logger.warning(f"OpenAI API returned {response.status_code}. Retrying in {sleep_duration:.2f}s...")
        ...
        raise BrainError(f"OpenAI API failed after {self.max_retries} retries. Last error: {last_error}")

    def embed(self, text: str) -> list[float]:
        raise NotImplementedError("Embeddings should be handled by FastEmbed via EmbeddingEngine, not OpenAI.")
