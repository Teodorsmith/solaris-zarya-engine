...
class TestReasoningMemory(unittest.TestCase):
    ...
    def _make_episode(self, outcome="failure", domain="math", verified=False):
        return ReasoningEpisode(
            state="2+2", hypothesis="5", action="submit", observation="wrong",
            outcome_class=outcome, reasoning_domain=domain, verified=verified
        )
    ...
    def test_mark_verified(self):
        ...
        row = self.memory.conn.execute("SELECT verified, srt_json FROM reasoning_episodes WHERE id=?", (ep_id,)).fetchone()
