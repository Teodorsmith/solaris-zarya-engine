...
def test_governor_depth_caps():
    ...
    with patch("builtins.input", return_value="y"):
        assert (
            gov.request_permission("action", g4, all_goals, is_autonomous=False) is True
        )
    ...
    with patch("builtins.input", return_value="n"):
        assert (
            gov.request_permission("action", g4, all_goals, is_autonomous=False)
            is False
        )
...
def test_governor_skill_write():
    ...
    assert (
        gov.request_skill_write_permission(
            "calc_fib", "skills/calc_fib.py", "def execute(): ...", is_autonomous=True
        )
        is False
    )
    assert "DENIED" in episodic.logs[-1].content
...
def test_governor_file_write():
    ...
    # Autonomous mode auto-denies
    assert (
        gov.request_file_write_permission("test.txt", "content", is_autonomous=True)
        is False
    )
    assert "DENIED" in episodic.logs[-1].content
    ...
    with patch("builtins.input", return_value="y"):
        assert (
            gov.request_file_write_permission(
                "test.txt", "content", goal_description="Write test file"
            )
            is True
        )
        assert "USER_APPROVED" in episodic.logs[-1].content
