...
from agent.config import REASONING_SUITE_DIR, ZPD_CATEGORIES, ZPD_MAX_ROUNDS, ZPD_DIFFICULTY_MIN, ZPD_DIFFICULTY_MAX
...
class ZPDRunner:
    ...
    def __init__(self, brain: BaseBrain) -> None:
        self.brain = brain
    ...
    def _run_problem(self, prob: dict[str, Any]) -> bool:
        ...
        raw_response = self.brain.generate(
        ...
        if expected_ans.lower() not in srt.conclusion.lower():
            logger.debug("ZPDRunner: Wrong answer (got %s, expected %s)", srt.conclusion, expected_ans)
            return False
